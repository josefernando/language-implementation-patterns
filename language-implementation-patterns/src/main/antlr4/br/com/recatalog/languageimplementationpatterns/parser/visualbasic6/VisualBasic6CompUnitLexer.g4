...
// public int HIDDEN_COMMENT = 3;
 ...
 public boolean ignoreKeyword = false;
 public boolean insideTypeDef = false;
 public boolean insideProcedureDef = false;
 //public boolean insideDeclareDef = false;
 
 
 public boolean setInsideTypeDef(){
//    System.err.println("insideTypeDef:" + insideTypeDef);
//    System.err.println("insideProcedureDef:" + insideProcedureDef);
    
 	if (!insideProcedureDef ) insideTypeDef = true; // Se estiver dentro de Sub Ou Function então é falso
 
 //   System.err.println("insideTypeDef2:" + insideTypeDef);
//    System.err.println("insideProcedureDef2:" + insideProcedureDef);
    
    return insideTypeDef;
 }
 
  
 public boolean setInsideProcedureDef(){
//    System.err.println("insideTypeDef:" + insideTypeDef);
//    System.err.println("insideProcedureDef:" + insideProcedureDef);
...
END_FUNCTION : E N D   WS+   F U N C T I O N {insideProcedureDef = false;};
END_IF : E N D   WS+   I F ;
END_IF_DIRECTIVE : '#' E N D    WS+   I F ; 
END_PROPERTY : E N D   WS+   P R O P E R T Y ;
END_SELECT : E N D   WS+   S E L E C T ;
END_SUB : E N D   WS+   S U B  {insideProcedureDef = false;};
END_TYPE : E N D   WS+   T Y P E {insideTypeDef = false;}
;
...
BEEP : B E E P ;
BEGIN : B E G I N -> pushMode(BeginMode) ;
BINARY : B I N A R Y ;
...
CONST :  C O N S T ;
DATE : D A T E ;
DECLARE : D E C L A R E  { pushMode(DeclareProcedureMode) ;} ;
...
// Para funcionar mudei de "mode" para "pushMode"e o ";" na linha seguinte. Não entendi o porquê!!

DIM : D I M  -> pushMode(VarDefMode)
 ;
...
FLOAT :  F L O A T ;
FOR : F O R ;
FUNCTION1 : F U N C T I O N {setType(FUNCTION); insideProcedureDef = true; };
GET : G E T ;
GLOBAL : G L O B A L ;
GOSUB : G O S U B  ;
GOTO : G O T O ;
...
STOP : S T O P ;
STRING :  S T R I N G ;
SUB1 : S U B {setType(SUB) ; insideProcedureDef = true;};
TEXT :  T E X T ;
THEN : T H E N  ;
TIME : T I M E  ;
TO : T O;
TRUE : T R U E ;
TYPE : T Y P E {if (setInsideTypeDef()) pushMode(VarDefMode);}
 ; 
TYPEOF : T Y P E O F ;
UNLOAD : U N L O A D ;
UNLOCK :  U N L O C K ;
...
BEGINPROPERTY1 : B E G I N P R O P E R T Y -> type(BEGINPROPERTY);
ENDPROPERTY1 : E N D P R O P E R T Y -> type(ENDPROPERTY);  
...
SEMI_CHAR : ',' ;
SEMI_COLON_CHAR : ';' ;
DOT_CHAR1 :   '.' -> type(DOT_CHAR);
LPAREN_CHAR1 : '(' -> type(LPAREN_CHAR);
RPAREN_CHAR1 : ')' -> type(RPAREN_CHAR);
EXPONENT_CHAR : '^' ;
PLUS_CHAR : '+' ;
MINUS_CHAR : '-' ;
MULT_CHAR : '*' ;
...
COMMENT : ( {getCharPositionInLine() == 0}?  R E M WS? (LINE_CONTINUATION | ~('\r' | '\n'))* '\r'? '\n'
            | '\'' (LINE_CONTINUATION | ~('\r' | '\n'))* 
          ) -> channel(HIDDEN_COMMENT) ; 
...
CURLY_LITERAL1 : '{' ~[}]*? '}' {startOfLine = false; setType(CURLY_LITERAL);} ;
DATELITERAL : '#' ~[#,\r\n]*? '#' {startOfLine = false;} ;
...
ID_TYPEINDICATOR : NAME_START_CHAR NAME_CHAR*  TYPE_INDICATOR {setType(IDENTIFIER);} ; 

FILENUMBER : '#' [a-zA-Z0-9_]+ ; 

NEWLINE1 : '\r'? '\n' {lineNumber = false; setType(NEWLINE);};
...
ID9 : NAME_START_CHAR NAME_CHAR* {setType(IDENTIFIER); popMode();}
 ;

ID_TYPEINDICATOR9 : NAME_START_CHAR NAME_CHAR*?  TYPE_INDICATOR  {setType(IDENTIFIER); popMode();}
;
...
mode DeclareProcedureMode;

FUNCTION10 : F U N C T I O N {setType(FUNCTION); popMode();};

SUB10 : S U B {setType(SUB); popMode();};
...
BEGINPROPERTY2 : B E G I N P R O P E R T Y -> type(BEGINPROPERTY) ;

END2 : E N D {setType(END) ; popMode();};

ENDPROPERTY2 : E N D P R O P E R T Y -> type(ENDPROPERTY) ;

DOT_CHAR2 :   '.' -> type(DOT_CHAR);

LPAREN_CHAR2 : '(' -> type(LPAREN_CHAR);
RPAREN_CHAR2 : ')' -> type(RPAREN_CHAR);

EQUAL_CHAR2 : '='  {setType(EQUAL_CHAR); mode(ValuePropertyMode);};

CURLY_LITERAL2 : '{' ~[}]*? '}' {startOfLine = false; setType(CURLY_LITERAL);} ;

INT2 : DIGIT+  -> type(INTEGER)  ;  

ID2 : NAME_START_CHAR NAME_CHAR* {lineNumber = false; setType(IDENTIFIER);} ;

// WS2 : [ \t]+ -> channel(HIDDEN_WHITESPACE) ;
 WS2 : [ \t] -> skip ;

NEWLINE2 : '\r'? '\n' { setType(NEWLINE); };


mode ValuePropertyMode;
           
//COMMENT3 : ( '\'' (LINE_CONTINUATION | ~('\r' | '\n'))* 
//            |   REM WS (LINE_CONTINUATION | ~('\r' | '\n'))*
//            |   REM '\r'? '\n') -> channel(HIDDEN_COMMENT) ;
...
// WS3 : [ \t]+ -> channel(HIDDEN_WHITESPACE) ;
WS3 : [ \t] -> skip ;

// BEGIN3 : B E G I N {setType(BEGIN) ; pushMode(BeginMode);};
...
// -> Não tem qualquer significado  VB.Net
//    type
//      %                 Integer
//      &                 Number Long
//      !                 Number Single precision  / collection operator
