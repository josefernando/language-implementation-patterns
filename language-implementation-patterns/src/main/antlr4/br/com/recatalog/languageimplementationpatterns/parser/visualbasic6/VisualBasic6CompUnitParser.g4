...
//   boolean inExpr = false;
   ...
   public void addKeyWordToVerify(String tokenImage) {
	...
	public void setInsideProcedure(Boolean inside){
		insideProcedure = inside;
	}
	
	public boolean isInsideProcedure(){
		return insideProcedure;
	}
...
variableDeclaration : 
	 {isInsideProcedure()}?	 modifier+? variableList  // Sempre tem Dim or Static
	 | {!isInsideProcedure()}?		 modifier*? variableList
;	 
...
methodType : 
	(  SUB
	| FUNCTION
	| propertyGetSet ) {setInsideProcedure(true);}
;
...
realParameter :
	RealFormalParam=expr   (EQUAL_CHAR | ':=') RealParam=expr |  RealParam=expr   
;
...
runTimeDependency : 
	OBJECT EQUAL_CHAR runTimeDependencyDeclaration 
;
...
formalParameters :
	...
	|   LPAREN_CHAR    formalParameter   (','   formalParameter)*   RPAREN_CHAR 
;
...
arrayDef :
 LPAREN_CHAR realParameterList? RPAREN_CHAR 
;
...
redimRange :
 LPAREN_CHAR expr TO expr RPAREN_CHAR
;
...
methodDefStmt : 
   ...
   blockStmt?
   endMethod   
   endOfStmt?     //pode ser  a última linha do arquivo
;
...
reDimStmt : 
		REDIM PRESERVE? variableRedimStmt  
;	 	  
...
endMethod : 
   (  END_SUB  
   | END_FUNCTION
   | END_PROPERTY  ) {setInsideProcedure(false); }
;
...
//	assignmentStmt   endOfStmt
	   expr EQUAL_CHAR VALUEPROPERTY endOfStmt+
;
...
guiAttribute : 
//	   assignmentStmt endOfStmt+
	   Property=expr EQUAL_CHAR Value=VALUEPROPERTY endOfStmt+
;

guiProperty : 
	BEGINPROPERTY Name=expr curlyLiteral? endOfStmt+?
		(guiProperty | guiAttribute)+?
	guiEndProperty endOfStmt+
;	
